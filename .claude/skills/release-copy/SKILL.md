...
**커뮤니티 글은 게시글 문법으로 쓴다** (MUST). 제품명 제목, `[주요 기능]` 같은 대괄호 목록, 단축키 나열, 리뷰·스타 요청은 스토어 문법이라 게시판에서는 광고로 읽혀 넘겨진다(이슈 [#196](https://github.com/kangkyunghyun/LinKHU/issues/196)). 첫 문장은 독자가 겪는 불편이고, 첫 문단 안에서 이게 무엇인지 드러나야 한다. 문체는 `~다.`체로 통일한다.
...
**두 문서 모두에서 확인한다.** 커뮤니티 글에는 업데이트 섹션도 기능 목록도 없지만 **승격은 그대로 필요하다** — 커뮤니티 글에서는 본문 문단에 한 문장으로 녹이는 것이 그 기능을 알릴 **유일한** 자리다. 다만 처음 보는 사람이 "그래서 어느 사이트가 열리는데"에 답할 수 없는 기능(단축키, 검색창 이동 등)은 녹이지 않는다. 두 문서에서 하는 일이 이렇게 갈린다.
...
sed -n '/^본문:/,/^```$/p' docs/community-post.md | grep -nE "^\[|합니다|습니다|해요|있어요"
```
