...
**운영 절차도 `spec/` 안에 있다.** 릴리스는 §7-5, 스토어 제출은 §7-6, 서비스 아이콘은 §4-6, 스크린샷은 §4-6, 문의 채널은 §5-3이다. 별도 가이드 폴더를 두지 않는다 — 절차와 그 절차가 지켜야 할 계약이 갈라지면 한쪽만 바뀐다.
...
1. 스펙은 코드와 함께 갱신한다. 어느 코드가 어느 문서와 짝인지는 [`spec/README.md`의 대응표](spec/README.md)가 정한다.
2. 결정 기록은 쓴 뒤 고치지 않는다. 결정이 바뀌면 새 번호로 덧붙이고 옛 항목의 상태만 바꾼다.
3. 작업 절차는 [AGENTS.md](../AGENTS.md)를 따른다. 이 문서에서 반복하지 않는다.
