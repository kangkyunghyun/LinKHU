...
LinKHU의 현재 동작과 그렇게 정한 이유, 그리고 운영 규격을 기록한다.
...
| 폴더 | 담는 것 | 언제 쓰는가 |
|---|---|---|
| [`spec/`](spec/README.md) | 배경·요구사항·설계·계약·구현 규약·운영 규격 | 코드를 바꿀 때마다 **함께 갱신한다** |
...
**운영 절차도 `spec/` 안에 있다.** 릴리스는 §7-5, 스토어 제출은 §7-6, 서비스 아이콘은 §4-5, 스크린샷은 §4-6, 문의 채널은 §5-3이다. 별도 가이드 폴더를 두지 않는다 — 절차와 그 절차가 지켜야 할 계약이 갈라지면 한쪽만 바뀐다.
