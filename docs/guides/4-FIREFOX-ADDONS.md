# 4. Firefox Add-ons 자동 배포

Firefox Add-ons 배포는 GitHub Actions의 `Publish Firefox Add-ons` 워크플로우에서 수동으로 실행한다 (MUST).

```text
§4-1   동작 범위                                       실행 조건과 업로드 범위
§4-2   필요한 GitHub Actions secrets                   배포 자격 증명
§4-3   Secret 발급 위치                                AMO 키 확인과 등록
§4-4   실행 방법                                       Actions 수동 실행
§4-5   실행 후 확인                                    검증 결과와 심사 상태
§4-6   사전 확인                                       릴리스와 등록 정보 확인
§4-7   참고 문서                                       공식 API 문서
```
...
## 4-2 필요한 GitHub Actions secrets
