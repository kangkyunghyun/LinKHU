...
- 공지 문구는 [Store Listing](../copy/1-STORE-LISTING.md)의 해당 버전 업데이트 섹션을 재사용한다 (SHOULD). 같은 내용을 따로 쓰지 않는다.
...
**알려진 간극:** [Store Release Checklist](../guides/2-STORE-RELEASE-CHECKLIST.md)는 "GitHub Release에 첨부된 ZIP을 내려받아 진행"하는 수동 절차를 기준으로 쓰여 있다. Whale 수동 배포에는 그대로 맞지만, Chrome·Firefox 자동 배포의 실제 동작과는 다르다. 두 경로가 같은 산출물을 만든다는 전제가 재현 가능한 패키징으로 보장되므로 당장 문제가 되지는 않는다. **체크리스트를 자동·수동 경로로 나눠 정리하는 것은 후속 과제다.**
...
인증 제공자의 앱 승인 상태가 검토·테스트 단계에 머물러 있으면 자격 증명 수명이 짧게 제한되는 경우가 있다. 반복 갱신을 없애는 근본 대응은 앱을 정식 상태로 전환하는 것이다. 갱신 절차와 필요한 값의 이름은 [Chrome 자동 배포 문서](../guides/3-CHROME-WEB-STORE.md)에 있다.
...
- 스토어 설명은 [Store Listing](../copy/1-STORE-LISTING.md)에서 원본을 관리하고, 버전마다 업데이트 섹션을 교체한다 (MUST).
...
| [Store Release Checklist](../guides/2-STORE-RELEASE-CHECKLIST.md) | 스토어 배포 공통 체크리스트 |
| [Store Listing](../copy/1-STORE-LISTING.md) | 스토어 설명 원본 |
| [Chrome Web Store 자동 배포](../guides/3-CHROME-WEB-STORE.md) | Chrome 워크플로 사용법 |
| [Firefox Add-ons 자동 배포](../guides/4-FIREFOX-ADDONS.md) | Firefox 워크플로 사용법 |
| [Whale Store 배포](../guides/5-WHALE-STORE.md) | Whale 수동 절차 |
| [문의 채널 설정](../guides/6-FEEDBACK-SETUP.md) | Google Form 연결 |
