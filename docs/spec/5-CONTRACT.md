...
**실패했을 때의 동작은 여기 적지 않는다**. 계약은 주고받기로 한 모양이고, 그 모양이 오지 않았을 때 화면이 무엇을 하는지는 §7-1 오류 처리가 정한다.
...
`services.json`은 다섯 필드를 그대로 직렬화한 배열이다. 여섯 번째 필드를 넣지 않는다 — §6-2-1의 스키마가 그대로 계약이다.
...
- 응답 `body`는 **신뢰 경계 밖이다**. `innerHTML`로 붙이지 않고 `createElement`와 `textContent`로만 조립한다. 근거와 나머지 세 규칙(`### Internal` 거르기, 마크다운 파서 없음, 실패 시 섹션 감춤)은 DECISIONS 5-3에 있다.
