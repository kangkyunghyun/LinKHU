...
**확장과 랜딩이 코드를 공유할 수 없다는 제약**([4-3-1](4-3-DESIGN-SOFTWARE-ARCHITECTURE.md))이 이 계약을 만든다. 같은 이유로 검색 랭킹과 릴리스 노트 필터가 각각 두 벌로 존재하고, 두 구현의 결과 일치를 테스트가 강제한다 ([7-2](7-2-TEST-CASES.md)).
