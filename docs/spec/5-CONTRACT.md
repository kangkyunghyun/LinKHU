...
LinKHU가 **자기 바깥과 주고받기로 약속한 것**이다. 안에서 어떻게 구현하든 이 약속을 어기면 상대가 깨진다.

카운터파트는 셋이다. 확장이 부르는 외부 API 둘(GitHub, Google Forms)과, 확장이 생성해 랜딩에 넘기는 산출물이다. 서버가 없으므로 자체 API는 없고, 따라서 여기에 적히는 것은 **우리가 통제할 수 없는 상대와의 약속**과 **두 화면이 코드를 공유할 수 없어 생긴 약속**뿐이다.
...
`services.json`은 다섯 필드를 그대로 직렬화한 배열이다. 여섯 번째 필드를 넣지 않는다. 데이터의 스키마가 그대로 계약이다.
...
**확장과 랜딩이 코드를 공유할 수 없다는 제약**이 이 계약을 만든다. 같은 이유로 검색 랭킹과 릴리스 노트 필터가 각각 두 벌로 존재하고, 두 구현의 결과 일치를 테스트가 강제한다.
...
- 호스트 권한은 이 **단 하나의 URL**로 좁혀 두었다.
- 확장의 조회는 12시간 캐싱한다. rate limit에 걸리면 모든 사용자의 버전 안내가 동시에 실패한다.
- 랜딩의 캐시는 `sessionStorage`에 탭 세션 단위로 둔다. 비인증 한도가 방문자 IP 기준 60회/시간인데 **학내망은 NAT로 IP를 공유하므로 한도가 집단으로 소진될 수 있다.**
- 응답 `body`는 **신뢰 경계 밖이다**. `innerHTML`로 붙이지 않고 `createElement`와 `textContent`로만 조립한다. 함께 지키는 규칙이 셋 더 있다 — `### Internal` 거르기, 마크다운 파서를 넣지 않기, 실패하면 섹션을 감추기.
...
- 폼 주소와 필드 id는 **공개돼도 안전한 값**이다. 노출되어도 남이 할 수 있는 일은 폼에 응답을 넣는 것뿐이다.
...
| 항목 | 값 | 어기면 |
| --- | --- | --- |
| 질문 1 | **장문형** `문의 내용`, **필수** | 필수가 아니면 외부에서 온 빈 요청이 그대로 저장된다 |
| 질문 2 | **단답형** `답변 받을 이메일`, 필수 아님 | — |
| 설정 탭의 "이메일 주소 수집" | **사용 안 함** | 익명 전송이 필수 필드 누락으로 저장되지 않는다 |

질문 1이 필수가 아닐 때 왜 위험한지가 이 계약의 핵심이다. `formUrl`은 공개 페이지의 JS에 그대로 있어 **누구나 `formResponse`에 직접 POST할 수 있고**, 필수가 아니면 Google이 빈 요청을 그대로 받는다. 확장과 랜딩이 빈 값을 막아도 그 경로는 우회된다.

### 확장에 연결하는 값

`src/feedback.js`의 `FEEDBACK_CONFIG`가 세 값을 갖는다. 랜딩도 같은 값을 쓴다.
...
설정 전까지 문의 버튼을 누르면 **문의 채널 미준비 안내**가 표시된다. `emailEntry`가 비어 있으면 이메일 입력란이 화면에 나타나지 않는다.
...
| 잘못된 것 | 결과 | 잡는 법 |
| --- | --- | --- |
| entry ID 오타 | 응답이 저장되지 않거나 빈 채로 저장 | 테스트 전송과 라이브 폼 점검 |
| `문의 내용`이 필수가 아님 | 외부에서 온 빈 요청이 그대로 저장 | 라이브 폼 점검 |
| "이메일 주소 수집" 켜짐 | 익명 전송이 저장되지 않음 | 테스트 전송 |

**설정을 바꿀 때마다 테스트 전송과 라이브 폼 점검을 함께 돌린다.**

### 응답을 받는 쪽

폼의 **응답** 탭 → ⋮ → **새 응답에 대한 이메일 알림 받기**를 켜 두면 문의가 들어올 때 폼 소유자에게 메일이 온다. **이 알림에는 문의 내용이 담기지 않는다** — 새 응답 안내와 링크뿐이며 설정으로 바꿀 수 없다. 내용까지 메일로 받으려면 Apps Script 트리거를 걸어야 하지만 **하지 않아도 된다**. 응답은 폼과 연결된 시트에 그대로 쌓이므로 유실되지 않는다.

## 5-4 공통 원칙

- **비밀 키가 필요한 방식은 채택할 수 없다**. 확장은 소스가 공개되므로 어떤 비밀값도 숨길 수 없다.
- 두 통신 모두 실패가 화면 사용을 막지 않아야 한다. 바로가기를 여는 것이 제품의 본체이고, 외부 호출은 전부 부가 기능이다.
- 새 외부 호출을 추가할 때는 네 가지를 같은 기준으로 적용한다 — 사용자에게 보일 것만 거르기, 파서 같은 의존성 늘리지 않기, 응답을 신뢰 경계 밖으로 다루기, 실패 시 조용한 폴백.
