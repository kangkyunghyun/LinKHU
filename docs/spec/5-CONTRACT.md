...
LinKHU가 **외부와 주고받는 값의 계약**이다. 내부 구현과 관계없이 이 약속을 어기면 상대가 깨진다.

카운터파트는 셋이다. 서버가 없으므로 자체 API는 없다.

| 상대 | 성격 |
| --- | --- |
| GitHub 릴리스 API | 통제할 수 없는 외부 서비스 |
| Google Forms | 통제할 수 없는 외부 서비스 |
| 랜딩이 받는 산출물 | 두 화면이 코드를 공유할 수 없어 생긴 약속 |
...
`src/data.js`가 서비스 정보의 단일 소스다. 랜딩 데이터는 여기서 **생성한 파생물**이므로 손으로 고치면 두 곳이 어긋난다.
...
확장과 랜딩이 코드를 공유할 수 없어 이 계약이 필요하다. 같은 이유로 검색 랭킹과 릴리스 노트 필터가 각각 두 벌로 존재하며, 테스트가 두 구현의 결과 일치를 강제한다.
...
응답에서 읽는 필드는 **넷뿐이다.** 나머지는 받아도 쓰지 않는다.
...
전송 주소는 공개 페이지에 그대로 있어 **누구나 직접 POST할 수 있다.** 질문 1이 필수가 아니면 Google이 빈 요청을 그대로 받으며, 확장과 랜딩이 빈 값을 막아도 그 경로는 우회된다.
...
폼의 **응답** 탭에서 새 응답 이메일 알림을 켜 둔다.

| 알아둘 것 | 내용 |
| --- | --- |
| 알림에 담기는 것 | 새 응답 안내와 링크뿐. **문의 내용은 담기지 않는다** |
| 바꿀 수 있나 | 없다. Google Forms 기본 동작이다 |
| 내용까지 받으려면 | Apps Script 트리거가 필요하다. **하지 않아도 된다** — 응답은 시트에 쌓이므로 유실되지 않는다 |

## 5-4 공통 원칙

- **비밀 키가 필요한 방식은 채택할 수 없다**. 확장은 소스가 공개되므로 어떤 비밀값도 숨길 수 없다.
- 두 통신 모두 실패가 화면 사용을 막지 않아야 한다. 바로가기를 여는 것이 제품의 본체이고, 외부 호출은 전부 부가 기능이다.
- 새 외부 호출에도 네 가지 기준을 적용한다. 사용자에게 보일 내용만 고르고, 파서 같은 의존성을 늘리지 않는다. 응답은 신뢰 경계 밖으로 다루며 실패하면 조용히 폴백한다.
