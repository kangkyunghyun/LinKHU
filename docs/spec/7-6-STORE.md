...
태그 push로 GitHub Release를 만든 뒤 세 스토어에 실제로 올린다.

세 스토어 모두 심사가 있으며 주기가 다르다. **같은 버전도 공개 시점이 서로 다르다.**
...
**자동 배포는 Release의 ZIP을 내려받지 않고 태그 시점의 소스로 다시 빌드한다.** 패키징이 재현 가능하므로 같은 태그의 ZIP은 릴리스 첨부물과 바이트 단위로 같아야 한다. Whale 수동 배포만 첨부된 ZIP을 직접 내려받아 쓴다.

**자동 배포 자격 증명은 만료된다**. 배포 워크플로의 인증 실패는 코드가 아니라 자격 증명 갱신 문제일 수 있다. 릴리스 당일에 발견하기 쉬우므로 오래 배포하지 않았다면 **릴리스 전에 배포 경로를 점검한다**.
...
**게시 상태는 경고 표시로 판별할 수 없다**. Google은 `https://www.googleapis.com/auth/chromewebstore`를 민감한 스코프로 분류한다. 프로덕션으로 전환해도 검증을 받지 않으면 "확인되지 않은 앱" 경고가 계속 나온다. 개인용 앱은 검증이 필요 없으며 **고급 → 안전하지 않은 페이지로 이동**으로 진행한다. Google Cloud Console의 OAuth 동의 화면 → 게시 상태 항목에서만 판별한다.
