...
- **코드에서 확인되는 사실만 적는다** (MUST). 문서를 쓰기 전에 실제 코드와 설정을 읽고 대조한다.
- **아직 없는 것은 쓰지 않는다** (MUST). 검토 중인 방향, 후속 과제, 미정 사항은 스펙이 아니라 이슈에 남긴다. 확정되고 구현되면 그때 이 문서에 들어온다. 결정되지 않은 것을 적어 두면 읽는 사람도 에이전트도 무엇을 따라야 할지 알 수 없다.
- **측정 가능하게 적는다** (MUST). "적절한 대비"가 아니라 "4.5:1 이상", "깜빡임 없음"이 아니라 "첫 페인트 전 표식 적용"처럼 확인할 수 있어야 한다.
- 구현과 스펙은 **같은 PR에서 함께** 바꾼다 (MUST). 그래서 `main`의 스펙은 언제나 `main`의 코드를 설명한다.

## 키워드

계약 문장은 다음 키워드로 강도를 구분한다.
...
**§2-1 사용자 스토리에는 이 키워드를 쓰지 않는다** (MUST). 그 문서는 사용자가 겪는 일을 사용자의 말로 적는 자리이고, 저장 키 이름·함수·내부 식별자도 쓰지 않는다. 같은 사실을 계약으로 못 박는 것은 4-x 설계와 6-x 구현이 맡는다.
...
4. **문서와 코드가 어긋나면 코드가 사실이다** (MUST). 문서를 고친다. 계약을 바꾸려는 것이라면 스펙과 구현을 같은 PR에서 함께 바꾼다.
5. 같은 내용을 두 곳에 쓰지 않는다 (MUST). 아래 원본 표에서 그 주제의 원본을 확인하고, 다른 문서에서는 번호로 가리킨다.
6. 설계 방향을 바꾸는 판단은 [`DECISIONS.md`](DECISIONS.md)에 배경·대안·채택·영향과 함께 남긴다 (MUST). 스펙에는 결과만 쓰고 앵커로 링크한다.
