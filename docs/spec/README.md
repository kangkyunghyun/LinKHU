...
- **코드에서 확인되는 사실만 적는다**. 문서를 쓰기 전에 실제 코드와 설정을 읽고 대조한다.
...
- **측정 가능하게 적는다.** "적절한 대비"가 아니라 "4.5:1 이상", "깜빡임 없음"이 아니라 "첫 페인트 전 표식 적용"처럼 확인할 수 있어야 한다.
...
- 구현과 스펙은 **같은 PR에서 함께** 바꾼다. 그래서 `main`의 스펙은 언제나 `main`의 코드를 설명한다.
