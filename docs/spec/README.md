...
이 폴더가 **저장소의 유일한 문서 원본**이다. 배경·요구사항·설계·계약·구현 규약·운영을 번호 계층으로 담는다. 결정의 배경과 대안은 [`DECISIONS.md`](DECISIONS.md)에 로그로 쌓는다. "어떻게 일하는가"(이슈·브랜치·커밋·PR 절차)는 [`AGENTS.md`](../../AGENTS.md)가 원본이므로 여기서 반복하지 않는다. 릴리스·스토어 제출·아이콘·스크린샷 같은 운영 규격도 이 번호 계층에 둔다.
...
**이 문서만 보고 구현해도 지금과 같은 결과가 나와야 한다.** 여기서 설계하고 코드가 따른다. 내부 변수 이름처럼 구현에서 정할 사항은 달라도 되지만, 기능·화면 치수·디자인 규칙·처리 순서는 모두 여기에 있어야 한다.

- **자연어로 적는다.** CSS 속성이나 함수 시그니처를 옮겨 적지 않는다. 무엇을 왜 정했는지 쓰고 그 결과값을 문장에 담는다.
...
파일별로 함께 볼 문서를 고정한다. **코드를 바꾸면 짝지어진 문서를 같은 PR에서 갱신한다**.
...
여러 문서에 흩어지기 쉬운 주제의 원본을 고정한다. **절차와 규칙은 원본에만 쓰고 다른 곳에서는 링크한다**.
