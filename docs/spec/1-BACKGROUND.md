...
LinKHU가 무엇이고, 어떤 문제를 어떤 방식으로 푸는지, 어디까지가 이 제품의 범위인지를 고정한다. 기술 스택과 디렉터리 구조도 여기에 둔다.
...
**이 표를 넘어서는 기능은 기능 판단이 아니라 제품 정체성 판단이다**. 특히 "하지 않는 것"을 하려면 권한이 늘거나 서버가 생기므로, DECISIONS에 결정을 남기고 시작한다.
...
팝업을 여는 단축키는 `commands`의 `_execute_action`에 **제안값으로** 등록한다. `suggested_key`는 기본 `Ctrl+Shift+L`, macOS는 `Command+Shift+L`이다. **제안일 뿐 등록이 보장되지는 않는다.** 다른 프로그램이 이미 쓰는 조합이면 등록되지 않고, 확장이 그 조합을 빼앗을 수도 없다.
