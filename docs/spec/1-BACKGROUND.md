...
기술 스택과 디렉터리 구조도 여기에 둔다. 구현 세부는 4-3 아키텍처에서 다루고, 이 문서는 "무엇 위에 서 있는가"까지만 기술한다.
...
- **사용자 행동 수집** — 분석 SDK, 텔레메트리, 원격 로그가 없다. 근거는 DECISIONS 1-3을 참고한다.
- **계정 기반 동기화** — 설정은 브라우저 로컬 저장소에만 남는다. 서버가 없다.
- **경희대학교 공식 서비스 지위** — LinKHU는 공식 산출물이 아니다. 공식 색상을 쓰되(4-2 UI 규칙), 대학을 사칭하지 않는다.
...
| **디자인 토큰(Design Token)** | `src/theme.css`의 `:root` CSS 변수. 색상 값의 단일 소스다. §4-2 참고. |
...
- **외부 공지·홍보에서 이 기능을 예정된 것으로 말하지 않는다** (MUST). 구현되지 않은 기능을 알리면 약속이 되고, 무산되면 신뢰를 잃는다. 릴리스 공지 규칙은 7-4 배포에 있다.
- 착수한다면 §1-3의 "하지 않는 것"과 충돌하는지 먼저 판단한다 (MUST). 특히 노출·클릭 집계가 필요해지는 순간 DECISIONS 1-3과 정면으로 부딪힌다. 집계 없이 운영할 수 있는 형태인지가 착수 조건이다.
- 팝업은 320px 폭의 도구 화면이다. 배너가 바로가기 조작을 밀어내면 안 된다 (§4-2).
