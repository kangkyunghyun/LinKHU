...
기술 스택과 디렉터리 구조도 여기에 둔다. 구현 세부는 [4-3 아키텍처](4-3-DESIGN-SOFTWARE-ARCHITECTURE.md)에서 다루고, 이 문서는 "무엇 위에 서 있는가"까지만 기술한다.
...
- **경희대학교 공식 서비스 지위** — LinKHU는 공식 산출물이 아니다. 공식 색상을 쓰되([4-2 UI 규칙](4-2-DESIGN-UI-SYSTEM.md)), 대학을 사칭하지 않는다.
...
| **디자인 토큰(Design Token)** | `src/theme.css`의 `:root` CSS 변수. 색상 값의 단일 소스다. [4-2](4-2-DESIGN-UI-SYSTEM.md) 참고. |
...
- 팝업은 320px 폭의 도구 화면이다. 배너가 바로가기 조작을 밀어내면 안 된다 ([4-2](4-2-DESIGN-UI-SYSTEM.md)).
