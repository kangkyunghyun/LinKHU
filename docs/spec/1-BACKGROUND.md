...
**빌드 단계가 없다.** 번들러도 트랜스파일러도 프레임워크도 쓰지 않고, `src/`의 정적 HTML/CSS/JavaScript가 그대로 확장 프로그램이 된다.

Node.js는 **개발 도구로만** 쓴다. 검증·생성·패키징 스크립트와 테스트가 여기서 돈다.
