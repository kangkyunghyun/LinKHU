...
- **계약을 바꾸려는 경우** — 이 문서를 먼저 고치고 검증기를 맞춘다. 스펙이 계약의 원본이다.
- **현행 동작을 기술하다 어긋난 경우** — 코드가 사실이므로 문서를 고친다. 사실 기록은 코드를 이긴 적이 없다.

즉 **"무엇이어야 하는가"는 이 문서가, "지금 무엇인가"는 검증기가 정한다.** 둘이 다르면 그 자체가 해결해야 할 간극이며, 어느 쪽이 앞서 있는지 문서에 밝힌다. 실제 사례가 §6-2-1의 필드 개수 규칙이다 — 스펙이 앞서 있고 검증기가 아직 따라오지 못했다.
...
`id`는 사용자의 `userOrder`에 저장되는 값이다. id를 바꾸면 그 서비스를 담아둔 **기존 사용자의 설정에서 항목이 조용히 사라진다.** 렌더링 단계가 `MASTER_SITE_LIST`에 없는 id를 걸러내기 때문이다(§4-4-2). 아이콘 파일명을 `id`와 맞추는 것은 권장이지 강제가 아니다.
...
`http://` 경고는 "고쳐야 할 일"이 아니라 **"확인이 필요한 항목"** 이라는 표시다. 2026-07-30 기준 16건이 있으며 모두 HTTPS 미지원을 확인한 의도된 잔존이다. 새 항목을 넣거나 주소를 바꿀 때 지원하는데도 `http://`로 넣으면 경고가 하나 늘고, **그 경고가 확인된 것인지 아닌지 구분할 수 없게 된다**.
...
- **여러 서비스가 같은 아이콘을 공유해도 된다**. 전용 도안이 없는 학과가 공통 아이콘을 쓰는 경우가 있다. 그래서 중복 제약이 없다 (DECISIONS 2-1).

### `category`

이슈 [#150](https://github.com/kangkyunghyun/LinKHU/issues/150)에서 구현했다.

여덟 값과 그 순서는 §3-2가 원본이고, 단일 소스는 `src/data.js`의 `SITE_CATEGORIES`다. 설정 페이지의 드롭존과 필터 칩이 이 배열에서 렌더되므로 값을 추가하면 화면이 따라오고, **배열 순서가 곧 설정 페이지의 영역 순서**다.
...
- 아이콘 파일은 **다른 서비스가 그 파일을 참조하지 않을 때만 지운다**. 아이콘은 공유될 수 있으므로(§6-2-3), 확인 없이 지우면 남아 있는 서비스의 아이콘이 깨진다. 2026-07-30 기준 `images/common/khu.png`를 세 서비스가 공유하고 있다.
